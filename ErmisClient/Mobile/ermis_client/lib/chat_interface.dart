...
class ChatsState extends LoadingState<Chats> {
  List<ChatSession> _conversations = <ChatSession>[];
    ...
    return Scaffold(
      ...
      floatingActionButton: Padding(
        ...
        child: FloatingActionButton(
          onPressed: () async {
            final String? input = await showInputDialog(context: context, title: "Send Chat Request", hintText: "Enter client id - e.g 117");
            ...
            Client.getInstance().getCommands.sendChatRequest(clientID);
          },
      ...
      body: RefreshIndicator( // if user scrolls downwards refresh chat requests
  ...
  Widget buildChatButton(int index) {
    return createOutlinedButton(
        context: context,
        text: _conversations[index].toString(),
        avatar: UserAvatar(imageBytes: _conversations[index].getMembers[0].getIcon, isOnline: _conversations[index].getMembers[0].isActive),
        onTap: () {
          Navigator.push(
            context,
            MaterialPageRoute(
  ...
  Future<void> _refreshContent() async {
    Client.getInstance().getCommands.fetchChatSessions();
    setState(() {
      isLoading = true;
    });
  }
        ...
        Positioned(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: isOnline ? Colors.green : Colors.red, // Online or offline color
...
class ChatRequestsState extends LoadingState<ChatRequests> {
  ...
  Widget buildChatRequestButton(int index) {
    return createOutlinedButton(
        context: context,
        text: _chatRequests[index].toString(),
        avatar: UserAvatar(imageBytes: Uint8List.fromList(List.empty()), isOnline: false),
        otherWidgets: [
          GestureDetector(
            onTap: () => Client.getInstance().getCommands.acceptChatRequest(_chatRequests[index].clientID),
            child: Icon(
              Icons.check,
          ...
          GestureDetector(
            onTap: () => Client.getInstance().getCommands.declineChatRequest(_chatRequests[index].clientID),
            child: Icon(
  ...
  Future<void> _refreshContent() async {
    print("asd");
    Client.getInstance().getCommands.fetchChatRequests();
    setState(() {
      isLoading = true;
    });
  }
